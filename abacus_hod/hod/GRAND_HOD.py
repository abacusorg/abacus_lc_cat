        ...
        for i in range(hstart[tid], hstart[tid + 1]):
            if keep[i] == 1:
                ...
                if rsd:
                    ...
                    proj = inv_velz2kms*(lrg_vx[j1]*nx+lrg_vy[j1]*ny+lrg_vz[j1]*nz)/(nx*nx+ny*ny+nz*nz)
                ...
                # rsd only applies to the z direction
                if rsd:
                    nx = elg_x[j1] - origin[0]
                    ny = elg_y[j1] - origin[1]
                    nz = elg_z[j1] - origin[2]
                    proj = inv_velz2kms*(elg_vx[j1]*nx+elg_vy[j1]*ny+elg_vz[j1]*nz)/(nx*nx+ny*ny+nz*nz)
                    elg_x[j1] = elg_x[j1]+proj*nx
                    elg_y[j1] = elg_y[j1]+proj*ny
                    elg_z[j1] = elg_z[j1]+proj*nz
                ...
                # rsd only applies to the z direction
                if rsd:
                    nx = qso_x[j1] - origin[0]
                    ny = qso_y[j1] - origin[1]
                    nz = qso_z[j1] - origin[2]
                    proj = inv_velz2kms*(qso_vx[j1]*nx+qso_vy[j1]*ny+qso_vz[j1]*nz)/(nx*nx+ny*ny+nz*nz)
                    qso_x[j1] = qso_x[j1]+proj*nx
                    qso_y[j1] = qso_y[j1]+proj*ny
                    qso_z[j1] = qso_z[j1]+proj*nz
