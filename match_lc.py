...
def main(sim_name, z_lowest, z_highest, light_cone_parent, catalog_parent, merger_parent, resume=False):
    ...
    if resume:
        ...
        assert (np.abs(z_last-z_highest) <= 6.0e-1), "Your recorded state is not for the currently requested redshift, can't resume from old. Last recorded state is z = %.3f"%z_last
    else:
        z_last = -1
    ...
    for step in range(step_start,step_stop+1):
        ...
        for i_choice, choice_fn in enumerate(choice_fns):
            ...
            for i in range(len(mt_fns)):
                ...
                inds_mt_pid = np.arange(len(mt_pid))[mt_origins == origin]
